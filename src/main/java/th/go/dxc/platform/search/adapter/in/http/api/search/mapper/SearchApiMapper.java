...
public final class SearchApiMapper {
    ...
    public static SearchRequest toDomain(String datasetId, SearchRequestDto dto) {
        ...
        return new SearchRequest(new Dataset.Id(datasetId), criteria, pageReq);
    }
     ...
     */
    public static SearchResultDto toDto(SearchResult r) {
        var page = r.pageResult(); // DomainPageResult<DataRecordSummary>

        var items = page.content().stream()
                .map(SearchApiMapper::toDtoItem)
                .toList();

        return SearchResultDto.of(items,page.number(),page.size(),page.totalElements());
        
    }

    private static SearchResultDto.Item toDtoItem(DataRecordSummary s) {
